...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    /**
     * Check if user is not a provider too
     */

    if (req.userId === provider_id) {
      return res
        .status(400)
        .json({ error: 'You can`t create appointment to yourself' });
    }
    ...
    if (isBefore(hourStart, new Date())) {
      return res.status(400).json({ error: 'Past date are noy permitted' });
    }
  ...
  async delete(req, res) {
    ...
    if (!appointment.cancelable) {
      return res.status(400).json({ error: 'Can`t cancel this appointment' });
    }
    ...
    if (isBefore(dateWithSub, new Date())) {
      return res
        .status(401)
        .json({ error: 'You can only cancel apppointmens 2 hours in advance' });
    }
